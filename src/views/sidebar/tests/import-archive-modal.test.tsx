...
function makeFile(name: string, sizeBytes: number = 0): File {
...
describe('ImportArchiveModal', () => {
	describe('file info block', () => {
		it('renders the file name', () => {
			...
			setupTest(<ImportArchiveModal folder={folder} file={makeFile('archive.tgz')} />);

			expect(screen.getByText('archive.tgz')).toBeInTheDocument();
		});
		...
		it('renders the TGZ file type description for a .tgz file', () => {
			...
			expect(screen.getByText('modal.import_archive.file_type.tgz')).toBeInTheDocument();
		});
		...
		it('does not show the TGZ description for a .mbox file', () => {
			...
			expect(screen.queryByText('modal.import_archive.file_type.tgz')).not.toBeInTheDocument();
		});
	...
	describe('file size formatting', () => {
		it('renders the size in bytes when smaller than 1 KB', () => {
			...
			setupTest(<ImportArchiveModal folder={folder} file={makeFile('archive.tgz', 500)} />);
		...
		it('renders the size in KB when between 1 KB and 1 MB', () => {
			const folder = generateFolder({ id: FOLDERS.INBOX });
			setupTest(
				<ImportArchiveModal folder={folder} file={makeFile('archive.tgz', 2 * 1024)} />
			);
		...
		it('renders the size in MB when between 1 MB and 1 GB', () => {
			const folder = generateFolder({ id: FOLDERS.INBOX });
			setupTest(
				<ImportArchiveModal folder={folder} file={makeFile('archive.tgz', 3_000_000)} />
			);
		...
		it('renders the size in GB when 1 GB or larger', () => {
			const folder = generateFolder({ id: FOLDERS.INBOX });
			setupTest(
				<ImportArchiveModal folder={folder} file={makeFile('archive.tgz', 2_000_000_000)} />
			);
