...
describe('SharedCalendarResponse component', () => {
	it('uses the plain folder name when it does not conflict with an existing top-level folder', async () => {
		...
		const sharedContent =
			...
			'  <link view="message" perm="rwidxa" name="Team Projects" id="2"/>\n' +
			'</share>';
		...
		expect(folderNameInput).toHaveValue('Team Projects');
	});

	it('appends the sharer name when the folder name conflicts with the existing Inbox folder', async () => {
		populateFoldersStore();
		const sharedContent =
			'<share action="new" version="0.2" xmlns="urn:zimbraShare">\n' +
			'  <grantee name="user@demo.test.io" id="657256aa-0f56-460a-9922-ff048da17af7" email="user@demo.test.io"/>\n' +
	...
	it('uses the plain calendar name when it does not conflict with an existing calendar', () => {
		populateFoldersStore();
		const name = 'Team Retro';
		...
		expect(folderNameInput).toHaveValue(name);
	});

	it('appends the name of the sharer when the calendar name conflicts with the existing default Calendar', () => {
		populateFoldersStore();
		const name = 'Calendar';
		const grantorName = 'grantor name';
		const sharedContent = `
	...
	it('uses the plain address book name when it does not conflict with an existing address book', () => {
		populateFoldersStore();
		const name = 'Contact';
		const grantorName = 'grantor name';
		const sharedContent = `
	...
	it('appends the name of the sharer when the address book name conflicts with the existing default Contacts', () => {
		populateFoldersStore();
		const name = 'Contacts';
		const grantorName = 'grantor name';
		const sharedContent = `
