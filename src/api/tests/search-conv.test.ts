...
describe('searchConvSoapApi', () => {
	...
	test('should include read=1 when read parameter is true', async () => {
		...
		searchConvSoapApi({ conversationId: '1', folderId: FOLDERS.INBOX, fetch: 'all', read: true });
	...
	test('should NOT include read parameter when read is false', async () => {
		...
		searchConvSoapApi({ conversationId: '1', folderId: FOLDERS.INBOX, fetch: 'all', read: false });
