...
export type ConvReplyAllUIAction = UIAction<
	UIActionExecutionParams & { conversation: Conversation },
	Conversation
>;

export const useUIActionConvReplyAll = (): ConvReplyAllUIAction => {
	const [t] = useTranslation();

	const canExecute = useCallback<NonNullable<ConvReplyAllUIAction['canExecute']>>(
		(conversation) => {
	...
	const execute = useCallback<NonNullable<ConvReplyAllUIAction['execute']>>(
		({ conversation }) => {
			...
			addBoard<BoardContext>({
				url: `${MAILS_ROUTE}/edit?action=${EditViewActions.REPLY_ALL}&id=${message?.id}`,
				title: ''
			});
		},
		[canExecute]
