...
import { ConvReplyUIAction, useUIActionConvReply } from '../conv-reply';

describe('useUIActionConvReply', () => {
	it('should return an object with specific data', () => {
		const { result: hookResult } = setupHook(useUIActionConvReply);
	...
	describe('execute', () => {
		it('Should not add a board if the action cannot be executed', () => {
			...
			const params: Parameters<NonNullable<ConvReplyUIAction['execute']>>[0] = {
				conversation
			};
		...
		it('Should add a board on a specific url', () => {
			...
			expect(addBoard).toBeCalledWith(
				expect.objectContaining({
					url: `mails/edit?action=reply&id=30`
				})
			);
		});
	});
