...
import { ConvReplyAllUIAction, useUIActionConvReplyAll } from '../conv-reply-all';

describe('useUIActionConvReplyAll', () => {
	it('should return an object with specific data', () => {
		const { result: hookResult } = setupHook(useUIActionConvReplyAll);
	...
	describe('execute', () => {
		it('Should not add a board if the action cannot be executed', () => {
			...
			const params: Parameters<NonNullable<ConvReplyAllUIAction['execute']>>[0] = {
				conversation
			};
		...
		it('Should add a board on a specific url', () => {
			...
			expect(addBoard).toBeCalledWith(
				expect.objectContaining({
					url: `mails/edit?action=replyAll&id=30`
				})
			);
		});
	});
